...
public class CompraDAO implements ICompraVenda<Compra> {
	...
	private String Select_Compra() {
		...
		sql.append(
				"select c.cod, c.data_origem, c.data_recebido, c.valor_total, c.status, c.cod_Fornecedor, c.cod_Funcionario from compra as c");
		return sql.toString();
	}
	...
	public List<Compra> Buscar(Date DataInicio, Date DataFim, String status) throws Exception {
		...
		try {
			...
			sql.append(
					" and c.data_origem >= '" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(DataInicio) + "'");
	...
	public List<Compra> Buscar(Date DataInicio, Date DataFim, String status, int codFuncionario, int codFornecedor)
			throws Exception {
		List<Compra> lista = new ArrayList<Compra>();
		...
		try {
			...
			if (codFuncionario > 0) {
				sql.append(" and c.cod_Funcionario = " + codFuncionario);
			}

			if (codFornecedor > 0) {
				sql.append(" and c.cod_Fornecedor = " + codFornecedor);
			}
	...
	private int ProximoCodCompra() throws SQLException {
		...
		try {
			...
			if (retorno < 0)
				throw new Error("Não foi possível recuperar o proximo número dos produtos");
