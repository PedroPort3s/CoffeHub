		...
		try {
			...
			if (sqlEx.getErrorCode() == 1451) {
				throw new SQLException(
						"Esta categoria esta sendo utilizada por um ou mais produtos, para deletar a mesma, favor deletar os produtos antes.");
			} else {
				throw sqlEx;
			}
		} catch (Exception ex) {
	...
	private int ProximoNumeroCategoria() throws Exception {
		...
		try {
			...
			PreparedStatement statement = conexao
					.prepareStatement("select ifnull(max(cod),0) as 'maior' from categoria");
			...
			if (numeroCategoria < 0)
				throw new Exception("Não foi possível recuperar o proximo número da categoria");
